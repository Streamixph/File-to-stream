...
async def handle_file_upload(message: Message, user_id: int):
    try:
        ...
        button = InlineKeyboardMarkup([[InlineKeyboardButton(text="Open Your Link 🔗", url=final_link)]])
        await message.reply_text(text=f"✅ Your shareable link has been generated!", reply_markup=button, quote=True)
    except Exception as e:
